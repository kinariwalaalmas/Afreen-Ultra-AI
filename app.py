...
# 2. Importing Super-Powers (Ensure names match exactly in other files)
...
# 4. TOP UI (News Ticker & Audio Auto-Play)
...
# 5. 🛠️ MODERN TOOLBAR (Plus Menu & Green Mic)
...
with col_plus:
    # ➕ Popover Menu for Advanced Tools
    with st.popover("➕"):
        st.write("### 🛠️ Afreen Super Tools")
        cam_tool = st.file_uploader("📷 Fashion Scan", type=['jpg','png'], key="cam_final_v1")
        id_search = st.text_input("🔍 Search ID/Number", placeholder="Type number here...", key="id_final_v1")
        url_tool = st.text_input("🔗 URL Analyze", placeholder="Paste link here...", key="url_final_v1")

with col_mic:
    # 🟢 GREEN MIC (Styling handled in ui_power.py)
    audio = mic_recorder(start_prompt="🎙️", stop_prompt="🛑", key='mic_final_v1')
...
if audio and audio.get('bytes'):
    ...
    input_q = f"SEARCH_DATA: {id_search}"
elif url_tool:
    input_q = f"ANALYZE_URL: {url_tool}"
elif txt:
    input_q = txt

# 7. 📢 EXECUTION ENGINE (Anti-Loop Logic)
if input_q and input_q != st.session_state.last_processed:
    ...
    with st.spinner("Afreen is processing..."):
        ...
        # 🔍 Intelligence Category (Search/URL/ID)
        else:
